...
class Pengeluaran extends Admin_Controller {
    ...
    {
        $this->vars['user']     = user();
        $this->vars['judul']    = 'Data Pengeluaran';
        $this->vars['tahun']    = $this->Pengeluaran_m->get_tahun();
        $this->vars['bulan']    = $this->Pengeluaran_m->get_bulan();
	    $this->load->view('admin/data/pengeluaran/index', $this->vars);
    }
    ...
    {      
        ...
        if($bulanawal != 'Pilih bulan' && $bulanakhir != 'Pilih bulan' && $tahun1 != 'Pilih tahun' && $bulanawal <= $bulanakhir){
            ...
            $data['pengeluaran'] = $this->Pengeluaran_m->get($tahun1, $bulanawal, $bulanakhir);
            $data['total'] = $this->Pengeluaran_m->get_sum($tahun1, $bulanawal, $bulanakhir);
            $data['bulanawal'] = $bulanawal;
            $data['bulanakhir'] = $bulanakhir; 
            $data['tahun'] = $tahun1;
            $filename = 'Laporan-Pengeluaran-Bulan-'.date('F').'-Tahun-'.date('Y');
            $html1 = $this->load->view('admin/cetak/pengeluaran/index', $data, true);
            ...
            redirect('admin/data/pengeluaran');
        }
    ...
    public function form_impor()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            $msg = [
                    'sukses' => $this->load->view('admin/modal/impor-pengeluaran', $data, true)
            ];
    ...
    public function form_edit()
    {
        if ($this->input->is_ajax_request() == true) {
            $id = $this->input->post('id', true);
            $ambil_data = $this->Pengeluaran_m->get_data_by_id($id);
            ...
            $msg = [
                    'sukses' => $this->load->view('admin/modal/edit-pengeluaran', $data, true)
            ];
            echo json_encode($msg);
        } else{
            redirect('error_');
    ...
    public function form_view()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            $msg = [
                    'sukses' => $this->load->view('admin/modal/view-pengeluaran', $data, true)
            ];
            echo json_encode($msg);
        } else{
            redirect('error_');
    ...
    public function tambah()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            if ($this->form_validation->run() == true) {
                $this->_tambah();
                $msg = [
                        'sukses' => 'Data Pengeluaran berhasil ditambah.'
                ];
            } else {
                $msg = [
    ...
    private function _tambah()
    {
        ...
        $this->Pengeluaran_m->tambah($data);
    ...
    public function edit()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $id = $_POST['id'];
                $ambil_data = $this->Pengeluaran_m->get_data_by_id($id)->row();
                ...
                if( $upload_invoice ) {
                    ...
                    if( $this->upload->do_upload('invoice') ) {
                        ...
                        $msg = [
                                'sukses' => 'Data Pengeluaran berhasil diedit.'
                        ];
                    } else {
                        $msg = [
                                    'error' => '<div class="alert alert-danger alert-has-icon alert-dismissible show fade">
    ...
    private function _edit($new_invoice)
    {
        ...
        $this->Pengeluaran_m->edit($data, $id);
    }
            ...
            if( $upload_csv ) {
                ...
                if( $this->upload->do_upload('csv') ) {
                    ...
                    foreach ($csv as $row) {
                        ...
                        $msg = [
                                'sukses' => 'Data Pengeluaran berhasil di impor.'
                        ];
                    }
    ...
    function get_sum()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            $data = $this->Pengeluaran_m->get_sum($tahun1, $bulanawal, $bulanakhir);
    ...
    function get_tabel()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            $list = $this->Pengeluaran_m->get_datatables($tahun1, $bulanawal, $bulanakhir);
            $data = array();
            $no = $_POST['start'];
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->Pengeluaran_m->count_all(),
                "recordsFiltered" => $this->Pengeluaran_m->count_filtered($tahun1, $bulanawal, $bulanakhir),
                "data" => $data,
            );
