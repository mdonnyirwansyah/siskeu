...
class Pengeluaran_m extends CI_Model {
    ...
    var $column_order = array(null, 'kode', 'tanggal', 'uraian', 'penerima', 'kebutuhan', 'satuan', 'volume', 'keterangan');
    var $column_search = array('tanggal', 'uraian', 'penerima', 'kebutuhan', 'satuan', 'volume', 'keterangan');
   ...
   public function get($tahun1, $bulanawal, $bulanakhir)
    {
        $this->db->select('*, (satuan * volume) AS jumlah');
    ...
    public function get_pie()
    {
        $tahun = date('Y');
        $bulan = date('m');
        $this->db->select('SUM(satuan * volume) AS sum, uraian');
        $this->db->from($this->table);
        $this->db->where('YEAR(tanggal) = "'.$tahun.'" AND MONTH(tanggal) BETWEEN "'.$bulan.'" AND "'.$bulan.'" AND jenis = "'.$this->jenis.'"');
        $this->db->group_by('uraian');
        $this->db->order_by('sum', 'asc');
    ...
    public function hapus_banyak($id, $jml_data)
    {
        for ($i=0; $i < $jml_data; $i++) { 
            $this->db->delete($this->table, ['kode' => $id[$i]]);
