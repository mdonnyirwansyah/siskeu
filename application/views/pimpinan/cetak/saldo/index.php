		...
		<tr class="bordered">
			...
			<td class="bordered" style="font-size:16pt;">
				<center><strong>
					...
					<?= $bulanakhir != $bulanawal && $bulanakhir == '12' ? ' SAMPAI DESEMBER' : '' ?> TAHUN <?= $tahun ?><br>
				</strong></center>
			</td>
		</tr>
	</table>
	...
	<table style="width:100%;margin: auto;" class="border">
		<thead style="font-size:12pt; background-color: #A9A9A9;">
		  <tr>
			...
			<th class="border" style="text-align: center; width: 15%">Debit</th>
			<th class="border" style="text-align: center; width: 15%">Kredit</th>
			<th class="border" style="text-align: center; width: 15%">Saldo</th>
			<th class="border" style="text-align: center; width: 15%">Keterangan</th>
		  </tr>
		</thead>
		<tbody style="font-size:10pt;">
		  <?php $i = 1; $saldo = 0; foreach ($Saldo as $row) : ?>
		  	<?php if ($row->jenis == 'd') {
                    $debit = $row->satuan * $row->volume;
                    $kredit = 0;
                }elseif ($row->jenis == 'k') {
                    $debit = 0;
                    $kredit = $row->satuan * $row->volume;
                }
		  ...
		  <tr>
			...
			<td class="border">Rp <?= number_format($debit,0,',','.') ?>,-</td>
			<td class="border">Rp <?= number_format($kredit,0,',','.') ?>,-</td>
			<td class="border">Rp <?= number_format($saldo,0,',','.') ?>,-</td>
		...
		<tfoot>
          <tr>
            ...
            <th class="border" style="text-align: left;">Rp <?= number_format($sumDebit->sum,0,',','.') ?>,-</th>
            <th class="border" style="text-align: left;">Rp <?= number_format($sumKredit->sum,0,',','.') ?>,-</th>
            <th class="border" style="text-align: left;">Rp <?= number_format($sumDebit->sum - $sumKredit->sum,0,',','.') ?>,-</th>
            <th class="border"></th>
          </tr>
        </tfoot>
	</table>
